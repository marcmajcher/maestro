...
function generateMaestroCasting() {
  ...
  var director1 = range.getCell(17, 2).getValue();
  var director2 = range.getCell(18, 2).getValue();
  var musician = range.getCell(15, 2).getValue();
  var signup = range.getCell(22, 2).getValue();
  ...
  message += "Maestro is the Hideout's Saturday 10pm! It features games and scenes and nail-biting eliminations!\n\n";
  message += "Directors: "+director1+" and "+director2+"!\n";
  message += "Music: "+musician+"!\n\n";
  message += "Sign up via the Google Form below with your NAME, your EXPERIENCE LEVEL, and your FAVORITE WARMUP.\n\n";
  message += "(You'll only be cast for tech if you volunteer to do tech. Also, if you tech, you're automatically cast in the next show!)\n\n";
  message += "v v v v v SIGN UP HERE! v v v v v\n";
  message += signup+"\n";
  message += "^ ^ ^ ^ ^ SIGN UP HERE! ^ ^ ^ ^ ^\n\n"
  message += "_ .... .. ... .. ... _. ___ _ ._ ... .__. . _._. .. ._ ._.. __ . ... ... ._ __. . \n\n";
  message += "If you sign up, please consider the following:\n";
  message += "1. Call-time is a hard 9:20pm. Please do not be late. Warm-ups will be organized and led by a director or a pre-selected member of the cast.\n";
  message += "2. If you are in (or are planning on seeing) the 8pm mainstage, please mention that. I will try to only cast one or two players involved with the 8pm mainstage, since it forces them to miss call-time.\n";
  message += "3. Staying for notes is required. We are working hard for Maestro to get out by 11:45 and for notes to last no longer than 15 minutes. Feel free to drink your free beer and watch the show if eliminated!\n";
  message += "4. Dropping out of Maestro after getting cast results in a one month no-play penalty\n";

  showMessage("Maestro Casting Notice", message, fbURL);
}
...
function generateMaestroRawCasting() {
  ...
  var showDate = sheet.getName();
  var director1 = range.getCell(17, 7).getValue();
  var director2 = range.getCell(18, 7).getValue();
  var musician = range.getCell(15, 7).getValue();
  var signup = range.getCell(22, 7).getValue();
  ...
  message += "Sign up if you are a current or recent graduate of the Hideout Theatre, and bring your people!\n\n";
  message += "Call time is 5:20. Warm-ups will be at 5:30. Show is at 6pm! (it is ill-advised to be late).\n\n";
  message += musician+" will be our musician this week!!\n\n";
  message += "TAKE ADVANTAGE OF THIS AMPLE STAGE TIME!!!\n\n";
  message += "(Sign up with your full name and experience level via the form below. I can't cast you if you don't put your name and experience level.)\n\n";
  message += "v v v v v SIGN UP HERE! v v v v v\n";
  message += signup+"\n";
  message += "^ ^ ^ ^ ^ SIGN UP HERE! ^ ^ ^ ^ ^\n\n"

  showMessage("Maestro RAW Casting Notice", message, fbURL);
}
...
function generateMaestroCast() {
  ...
  var director2 = range.getCell(18, 2).getValue();
  var musician = range.getCell(15, 2).getValue();
  var tech = range.getCell(14, 2).getValue();
  var postURL = range.getCell(20, 2).getValue();
  ...
  var message = "Your Maestro cast for this week, " + showDate + ":\n\n";
  message += "Directors - "+director1+" and "+director2+"\n";
  message += "Music by "+musician+"\n";
  message += tech+" on tech\n\nPlayers:\n";

  // Sort and grab the cast list
  var players = sheet.getRange("B1:B12").sort(2).getValues();
  for (var i=0; i<players.length; i++) {
    message += players[i][0] + "\n";
  }

  message += "\nCall time is 9:20 - come on down and ready to warm up!"
  showMessage("Maestro Cast Notice", message, postURL);
};
...
function generateMaestroRawCast() {
  ...
  var director2 = range.getCell(18, 7).getValue();
  var musician = range.getCell(15, 7).getValue();
  var tech = range.getCell(14, 7).getValue();
  var postURL = range.getCell(20, 7).getValue();

  // Bail if it's not a casting sheet
  if (range.getCell(1, 6).getValue() !== 'RAW Cast') {
    return;
  }
  if (!director1 || !director2) {
    SpreadsheetApp.getUi().alert("Director Missing!");
  ...
  var message = "Our Maestro RAW cast for this week, " + showDate + "!\n\n";
  message += "Directors: "+director1+" and "+director2+"\n";
  message += tech+" on tech\n";
  message += musician+" on keyboard\n\nPlayers:\n";

  // Sort and grab the cast list
  var players = sheet.getRange("G1:G12").sort(7).getValues();
  for (var i=0; i<players.length; i++) {
    message += players[i][0] + "\n";
  }

  message += "\nCall time is 5:20 - warm it up and have fun!"
  showMessage("Maestro RAW Cast Notice", message, postURL);
}
...
function addNewWeek() {
  ...
  var schedule = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Schedule').getDataRange().getValues();
  for (var i=1; i<schedule.length; i++) {
    var thisDate = new Date(schedule[i][0]);
    var thisDateString = (thisDate.getMonth()+1)+'/'+thisDate.getDate()+'/'+thisDate.getFullYear();
...
function maestroReminderEmail() {
  ...
  var musician = sheet.getRange("B15").getValue();
  var director1 = sheet.getRange("B17").getValue();
  var director2 = sheet.getRange("B18").getValue();
  ...
  for (var i=0; i<directorContacts.length; i++) {
    if (directorContacts[i][0].match(director1)) {
      director1Email = directorContacts[i][0];
    }
    if (directorContacts[i][0].match(director2)) {
      director2Email = directorContacts[i][0];
    }
  ...
  var message = "Hey "+getFirstName(houseManagerEmail)+", "+getFirstName(director1)+", ";
  message += getFirstName(director2)+", and "+getFirstName(musician)+"!\n\n";
  message += getFirstName(houseManagerEmail) + " is managing house, and "+getFirstName(musician)+" is our musician!\n\n";
  message += getFirstName(director1)+", "+getFirstName(director2)+", call is at 9:20 - take a look at what people like for warmups, ";
  message += "and make sure they get into them!\n\n"+sheet.getRange("B23").getValue()+"\n\nYour cast:\n\n";
  message += "Tech: "+sheet.getRange("B14").getValue()+"\n\nPlayers:\n";
  ...
  var recipients = [maestroCzarEmail, houseManagerEmail, director1Email, director2Email, musicianEmail].join(",");

  var ui = SpreadsheetApp.getUi();
  var result = ui.alert("Sending to: "+recipients, message, ui.ButtonSet.OK_CANCEL);

  if (result == ui.Button.OK) {
    GmailApp.sendEmail(recipients, "You're involved in Maestro on "+sheet.getName()+"!", message);
...
function maestroRawReminderEmail() {
  ...
  var houseManagerEmail = contacts.getRange("B2").getValue();
  var musician = sheet.getRange("G15").getValue();
  var director1 = sheet.getRange("G17").getValue();
  var director2 = sheet.getRange("G18").getValue();
  ...
  message += getFirstName(director2)+", and "+getFirstName(musician)+"!\n\n";
  message += getFirstName(houseManagerEmail) + " is managing house, and "+getFirstName(musician)+" is our musician!\n\n";
  message += getFirstName(director1)+", "+getFirstName(director2)+", call is at 5:20 - take a look at what people like for warmups, ";
  ...
  if (result == ui.Button.OK) {
    GmailApp.sendEmail(recipients, "You're involved in Maestro RAW on "+sheet.getName()+"!", message);
    SpreadsheetApp.getActiveSpreadsheet().toast("Message sent.");
  }
  else {
    SpreadsheetApp.getActiveSpreadsheet().toast("Message not sent.");
...
function showMessage(title, message, url) {
  ...
  /*
  var panel = UiApp.createApplication().setHeight(500).setWidth(450);
  panel.setTitle(title);
  var messageText = panel.createHTML("<pre>"+message+"</pre><hr>");
  var forumLink = panel.createAnchor("Post to forum", url);
  panel.add(panel.createVerticalPanel().add(messageText).add(forumLink));
  SpreadsheetApp.getActive().show(panel);
  */
}
